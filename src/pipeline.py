...
class PipelinePhaseExecution:
    ...
    def execute(self, data_package: DataPackage, callback: Callable[[DataPackage], None], error_callback: Union[Callable[[DataPackage], None], None] = None) -> None:
        start_time = time.time()
        ...
        def execute_phases() -> None:
            # nonlocal waiting_time, start_context, dp_phase_ex, data_package, start_time
            try:
                ...
                if self._mode == PhaseExecutionMode.ORDER_BY_SEQUENCE:
                    print(f"{data_package.data} finished: {data_package.phases[-1].sequence_number}")
                    self._push_finished_data_package(dp_phase_ex.sequence_number)
                    finished_data_packages = self._pop_finished_data_packages()
                    ...
                    with self._lock:
                        last_finished_sequence_number = self._get_last_finished_sequence_number()
                        if dp_phase_ex.sequence_number <= last_finished_sequence_number and data_package.success:
                            ...
                            self._remove_data(dp_phase_ex.sequence_number)
                            if not data_package.success:
                                error_callback(data_package)
                        else:
                            self._set_last_finished_sequence_number(dp_phase_ex.sequence_number)
    ...
    def _get_last_finished_sequence_number(self) -> int:
        """
        Returns the last finished sequence number.
        """
        return self._last_finished_sequence_number

    def _set_last_finished_sequence_number(self, sequence_number: int) -> None:
        """
    ...
    def _add_data(self, dp: DataPackage) -> None:
        with self._lock:
            self._data_packages[self._next_sequence_number] = dp
            self._next_sequence_number += 1
    
    def _remove_data(self, sequence_number: int) -> None:
        """
    ...
    def _push_finished_data_package(self, sequence_number: int) -> None:
        """
    ...
    def _pop_finished_data_packages(self) -> Dict[str, DataPackage]:
        """
    ...
    def execute(self, phases: List[PipelinePhaseExecution], dp: DataPackage, callback: Callable[[DataPackage], None], error_callback: Union[Callable[[DataPackage], None], None] = None) -> None:      
        ...
        def new_callback(dp: DataPackage) -> None:
            ...
            if len(phases_queue) > 0:
                ...
                phase.execute(dp, new_callback, error_callback)
                print(f"Task {dp.data} submitted to {phase._name}")
                return
